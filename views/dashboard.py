...
from auth import (      # Importing our database management functions
    clear_user_traces, get_latest_trace_timestamp, save_traces_bulk,
    get_all_traces, get_mapping_for_trace, get_fuzzy_session
)
...
def show_dashboard_view():
    ...
    if not traces_data:
        ...
        df = pd.DataFrame(columns=["trace_id", "user_id", "agent_id", "credits", "created_at", "input", "output", "session_id", "inspect"])
    else:
    ...
    if df.empty:
        display_df = pd.DataFrame(columns=["trace_id", "user_id", "agent_id", "credits", "created_at", "input", "output", "session_id", "inspect"])
