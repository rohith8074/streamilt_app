...
from auth import (      # Importing our database and settings functions
    ...
    get_all_users, update_user_limit
)
...
def show_settings_view():
    ...
    with st.container():
        ...
        if target_user:
            ...
            current_val = user_data['credit_limit'] if user_data['credit_limit'] is not None else float(settings.get("max_credits", 2.0))
            
            # Adjust their wallet.
            new_val = st.number_input(f"New Wallet Limit for {target_user} ($)", value=float(current_val), min_value=0.1, step=0.1)
