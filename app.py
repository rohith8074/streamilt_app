...
from auth import get_total_platform_credits  # Total cost spent by everyone on the platform
from auth import get_user_credits  # Checks how much money/credits a person has spent
from auth import get_user_limit  # Finds out how much a user is allowed to spend
...
if st.session_state.logged_in:
    with st.sidebar:
        ...
        st.markdown(
            f'<div style="text-align: center; padding: 0.4rem 0;"><img src="{logo_base64}" width="88"></div>',
            unsafe_allow_html=True,
        )

        # Display the current user's name (compact card).
        st.markdown(
            f'<div style="background: {CARD_BG}; padding: 0.4rem 0.6rem; border-radius: 8px; border: 1px solid {BORDER_COLOR}; margin: 0.2rem 0;"><span style="color: {SECONDARY_TEXT}; font-size: 0.68rem; text-transform: uppercase; letter-spacing: 0.04em;">User</span><br><span style="color: {TEXT_COLOR}; font-weight: 600; font-size: 0.85rem;">{st.session_state.username}</span></div>',
            unsafe_allow_html=True,
        )
        ...
        if not past_sessions:
            st.markdown(
                f'<div style="color: {SECONDARY_TEXT}; font-size: 0.78rem; text-align: center; padding: 0.35rem 0; font-style: italic;">No past chats yet</div>',
                unsafe_allow_html=True,
            )
        else:
            ...
            with st.container(height=300):
                for session in past_sessions:
                    ...
                    preview_text = (
                        session["preview"][:40] + "..." if len(session["preview"]) > 40 else session["preview"]
                    )

                    with cols[0]:
                        if st.button(
                            f"📄 {preview_text}", key=f"hist_{session['session_id']}", use_container_width=True
                        ):
