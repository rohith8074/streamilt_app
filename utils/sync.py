import os
from auth import (
    get_latest_trace_timestamp, save_traces_bulk,
    get_mapping_for_trace, get_fuzzy_session
)
...
def sync_user_activity(username, session_id):
    ...
    # 2. Fetch the latest receipts (traces) from the Lyzr cloud.
    api_traces = get_traces(
    ...
    for t in api_traces:
        ...
        if trace_id:
            base_trace_url = os.getenv("LYZR_TRACES_URL", "https://agent-prod.studio.lyzr.ai/v3/traces").replace("/v3/traces", "")
